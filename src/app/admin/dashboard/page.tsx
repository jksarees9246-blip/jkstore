...
export default function AdminDashboard() {
  ...
  async function uploadImage(): Promise<{ url: string; path: string }[]> {
  const filesData: { url: string; path: string }[] = [];
  ...
  for (const file of imageFile || []) {
    ...
    filesData.push({
      url: data.publicUrl,
      path: fileName, // ✅ important for delete
    });
  }
...
async function handleAddProduct() {
  ...
  try {
    ...
    const url = uploaded.map(img => img.url);
    const path = uploaded.map(img => img.path);
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      className="border p-6 rounded-xl mb-8 bg-white shadow">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {/* IMAGE UPLOAD */}
          <input className="border p-3 rounded-lg" type="file" multiple accept="image/*" onChange={handleImageChange} />
...
//       if (discountEnabled && discountAmt > 0) {
//         discountPercent = Number(
//           (((priceNum - discountAmt) / priceNum) * 100).toFixed(2)
//         );
//       }
...
//       await supabase.from("products").update(updates).eq("id", editId!);
