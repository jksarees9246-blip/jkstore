...
{/* Header */}
              ...
              return (
                ...
                <div className="relative w-full aspect-[3/4] rounded-lg overflow-hidden mb-2">
                 ...
                 {/* <Image
                  ...
                  <Swiper
                    modules={[Autoplay]}
                    autoplay={p.image_url && Array.isArray(p.image_url) && p.image_url.length > 1 ? { delay: 2000 } : false}
                    loop={!!(p.image_url && Array.isArray(p.image_url) && p.image_url.length >= 3)}

                  >
                    {Array.isArray(p.image_url) ? (
                      p.image_url.map((img, i) => (
                        img ? ( 
                        <SwiperSlide key={i}>
                          <Image
                            src={img}
                      ...
                      <SwiperSlide>
                        <Image
                          src={p.image_url && Array.isArray(p.image_url) ? p.image_url[0] : (typeof p.image_url === 'string' ? p.image_url : "/no-image.png")}
                          alt="product"
                          width={300}
                          height={300}
                          className="w-full h-auto object-cover rounded"
                            loading="eager" 
                             onClick={() => setSelectedImage(p.image_url && Array.isArray(p.image_url) ? p.image_url[0] : (typeof p.image_url === 'string' ? p.image_url : null))}
                        />
                      </SwiperSlide>
                    )}
                  </Swiper>
      ...
      {!loading && totalItems > 0 && (
        <div className="max-w-8xl mx-auto p-4">
          ...
          <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1} className="px-2 py-0.5 text-lg rounded border bg-white shrink-0">
            <span className="text-green-800 text-lg animate-pulse">◀</span>
            Prev
            </button>
        <div className="flex gap-1 overflow-x-auto max-w-[220px] px-1 scrollbar-hide">
          ...
          <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page >= totalPages} className="px-2 py-0.5 text-lg rounded border bg-white shrink-0 ">
              Next  
                <span className="text-green-800 text-lg animate-pulse">▶</span>
